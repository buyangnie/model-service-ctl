...
#   - 模型目录: /data/models/c4ai-command-08-2024
#######################################
...
MODEL_DIR="${MODEL_DIR:-/data/models/c4ai-command-08-2024}"
...
# vLLM has native Command-R model support, so keep model-specific parser flags
# empty by default. Enable trust-remote-code only if your local checkpoint
# requires custom code beyond the built-in integration.
